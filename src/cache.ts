...
export default class DiscordDungeonCache {
  ...
  async updateGuild (): Promise<void> {
    const response = await fetch(`https://api.discorddungeons.me/v3/guild/${process.env.DISCORD_DUNGEONS_GUILD_ID as string}`, {
      headers: { Authorization: process.env.DISCORD_DUNGEONS_API_KEY as string }
    })
  ...
  async updateDiscordMembers (client: Client): Promise<void> {
    const members = await client.guilds.resolve(process.env.DISCORD_GUILD_ID as string)?.members.fetch()
    ...
    for (const member of members.values()) {
      ...
      if (this.guild.elder.includes(member.user.id) || this.guild.owner === member.user.id) {
        if (member.roles.cache.has(process.env.DISCORD_ELDER_ROLE_ID as string)) await member.roles.add(process.env.DISCORD_ELDER_ROLE_ID as string)
      } else if (member.roles.cache.has(process.env.DISCORD_ELDER_ROLE_ID as string)) {
        await member.roles.remove(process.env.DISCORD_ELDER_ROLE_ID as string)
      }
      if (this.guild.members.includes(member.user.id)) {
        if (!member.roles.cache.has(process.env.DISCORD_MEMBER_ROLE_ID as string)) await member.roles.add(process.env.DISCORD_MEMBER_ROLE_ID as string)
        if (member.roles.cache.has(process.env.DISCORD_NO_GUILD_ROLE_ID as string)) await member.roles.remove(process.env.DISCORD_NO_GUILD_ROLE_ID as string)
        if (member.roles.cache.has(process.env.DISCORD_ALLIED_GUILD_ROLE_ID as string)) await member.roles.remove(process.env.DISCORD_ALLIED_GUILD_ROLE_ID as string)
        if (member.roles.cache.has(process.env.DISCORD_OTHER_GUILD_ROLE_ID as string)) await member.roles.remove(process.env.DISCORD_OTHER_GUILD_ROLE_ID as string)
      } else {
        if (member.roles.cache.has(process.env.DISCORD_MEMBER_ROLE_ID as string)) await member.roles.remove(process.env.DISCORD_MEMBER_ROLE_ID as string)
    ...
    if (unknownDiscordMembers.length > 0) {
      ...
      for (const unknownDiscordMember of unknownDiscordMemberList) {
        const member = client.guilds.resolve(process.env.DISCORD_GUILD_ID as string)?.members.resolve(unknownDiscordMember.id)
        if (member === undefined || member === null) continue
        if (unknownDiscordMember.guild === '') await member.roles.add(process.env.DISCORD_NO_GUILD_ROLE_ID as string)
        else if (member.roles.cache.has(process.env.DISCORD_ALLIED_GUILD_ROLE_ID as string)) await member.roles.remove(process.env.DISCORD_NO_GUILD_ROLE_ID as string)
        if (this.guild.allies.includes(unknownDiscordMember.guild)) await member.roles.add(process.env.DISCORD_ALLIED_GUILD_ROLE_ID as string)
        else {
          if (member.roles.cache.has(process.env.DISCORD_ALLIED_GUILD_ROLE_ID as string)) await member.roles.remove(process.env.DISCORD_ALLIED_GUILD_ROLE_ID as string)
          if (unknownDiscordMember.guild !== '' && unknownDiscordMember.guild !== this.guild.id) await member.roles.add(process.env.DISCORD_OTHER_GUILD_ROLE_ID as string)
          else if (member.roles.cache.has(process.env.DISCORD_OTHER_GUILD_ROLE_ID as string)) await member.roles.remove(process.env.DISCORD_OTHER_GUILD_ROLE_ID as string)
        }
